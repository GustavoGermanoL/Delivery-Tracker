...
@Component({
  ...
  template: `
    <main class="min-h-screen bg-gray-50 max-w-md mx-auto shadow-2xl relative flex flex-col">
      ...
      <div class="bg-blue-600 px-4 pt-4 pb-0 flex justify-end">
  ...
  onSaveEntry(newEntry: DayEntry) {
    ...
    this.workDayService.save(payload).subscribe({
      next: () => {
        ...
        // Se estiver filtrando, recarrega o filtro. Se não, recarrega o mês.
        this.toastService.show('Dia salvo com sucesso!', 'success');
        if (this.isFilterActive) {
            // Recarrega sem perder o filtro atual (poderíamos salvar start/end em variavel)
            // Mas por simplicidade, vou recarregar o mês atual ou você pode salvar as datas do filtro
             this.loadData(); // Simplificação: volta pro mês ao salvar
             this.isFilterActive = false; // Tira o filtro para evitar confusão
        } else {
            this.loadData();
        }
      },
      error: (err) => {this.toastService.show('Erro ao salvar. Tente novamente.', 'error');}
    });
  }
